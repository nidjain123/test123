...
public class VehicleParkingDaoProxyImpl implements VehicleParkingDaoProxy {
  ...
  public VehicleParkingLocation parkVehicle(final Vehicle vehicle) {
    ...
    for (int level = 0; level < parkingLot.size(); level++) {
      for (int slot = 0; slot < parkingLot.get(level).getSlots().size(); slot++) {
        ...
        if (parkingSlot.isFree() && parkingSlot.getType().toString()
            .equalsIgnoreCase(getParkingTypeByVehicle(vehicle.getType()).toString())) {
          parkingLocation = VehicleParkingLocation.builder().level(level)
  ...
  public boolean leaveSlot(final int level, final int slotNumber) {
    if (parkingLot.get(level) != null && parkingLot.get(level).getSlots() != null && !parkingLot.get(level).getSlots()
        .isEmpty()
  ...
  public List<String> getRegistrationNumbersWithColor(String color) {
    ...
    for (int level = 0; level < parkingLot.size(); level++) {
      for (int slot = 0; slot < parkingLot.get(level).getSlots().size(); slot++) {
        if (!parkingLot.get(level).getSlots().get(slot).isFree() && parkingLot.get(level).getSlots().get(slot)
            .getVehicle().getColor().equalsIgnoreCase(color)) {
          listofVehiclesWithGivenColor.add(parkingLot.get(level).getSlots().get(slot)
  ...
  public int getSlotNumberWithRegNumber(String registrationNumber) {
    for (int level = 0; level < parkingLot.size(); level++) {
      for (int slot = 0; slot < parkingLot.get(level).getSlots().size(); slot++) {
        if (!parkingLot.get(level).getSlots().get(slot).isFree() && parkingLot.get(level).getSlots().get(slot)
            .getVehicle().getRegistrationNumber().equalsIgnoreCase(registrationNumber)) {
          return slot;
        }
      }
    }
